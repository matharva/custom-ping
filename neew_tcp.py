    ...
    for i in range(runs):
        ...
        if human_output:
            ...
            if i == len(range(runs))-1:
                print('---' + host + 'tcp-latency statistics ---')
                if latency_points:
                    print(
                        len(latency_points) + " out of " + (i+1) +
                        "packets transmitted successfully"
                    )
                    print(
                        'rtt min/avg/max = ' + min(latency_points) + '/' + mean(latency_points) + '/' + max(latency_points) + 'ms',   # noqa: E501
                    )
                else:
                    print('All ' + runs + 'transmissions failed')
...
def _human_output(host: str, port: int, timeout: int, latency_point: float, seq_number: int):
    ...
    if latency_point:
        print(
            '{host}: tcp seq={seq_number} port={port} timeout={timeout} time={latency_point} ms'.format(
                host=host, seq_number=seq_number, port=port, timeout=timeout, latency_point=latency_point)
        )
    else:
        print('{host}: tcp seq={seq_number} port={port} timeout={timeout} failed'.format(
            host=host, seq_number=seq_number, port=port, timeout=timeout))
